param(
    ...
    [int]$MaxFolds = 1,
    [int]$SkipTestEval = 1,
    [int]$MarginalContrast = 1,
    [string]$PairContactLossWeight = "0.005",
    [string]$MarginalConsistencyWeight = "0.10",
    [string]$MarginalContrastWeight = "0.02",
    [string]$MarginalContrastMargin = "0.03",
    [int]$WarmupEpochs = 5,
    [int]$SmokeOnly = 0
)
...
if ($MarginalContrast -ne 0 -and $BatchSize -lt 2) {
    throw "Marginal partner contrast requires BatchSize >= 2."
}
if ($MaxFolds -lt 1 -or $MaxFolds -gt 5) {
    throw "MaxFolds must be between 1 and 5."
}
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $variant = if ($MarginalContrast -ne 0) { "contrast" } else { "control" }
    $OutputDir = "outputs_pcbind_v7_marginal_${variant}_seed$Seed"
}
...
if ($SmokeOnly -ne 0) {
    Write-Host "PC-BIND v7 marginal partner-contrast smoke-only run complete."
    exit 0
}
...
& (Join-Path $ProjectRoot "run_seed.ps1") `
    ...
    -PairMarginalContrast $MarginalContrast `
    -PairMarginalContrastWeight $MarginalContrastWeight `
    -PairMarginalContrastMargin $MarginalContrastMargin `
    -PairMarginalContrastWarmupEpochs $WarmupEpochs `
    -BatchSize $BatchSize
