param(
    ...
    [string]$MarginalConsistencyWeight = "0.02",
    [int]$WarmupEpochs = 5,
    [int]$SmokeOnly = 0
)
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $OutputDir = "outputs_pcbind_v6_consistency_seed$Seed"
}
...
if ($SmokeOnly -ne 0) {
    Write-Host "PC-BIND v6 marginal-consistency smoke-only run complete."
    exit 0
}
